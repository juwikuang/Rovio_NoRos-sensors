/*
 * Modified getImageList by Eric Chow
...
void getParameter(std::string cameParam, std::string imuParam,
		cameraParameters & camParam, noiseParameters & initNoiseParam) {
	...
	R_B_C << T_BS.at<double>(0, 0), T_BS.at<double>(0, 1), T_BS.at<double>(0,
			2),
	...
	camParam.Bp_c0 << T_BS.at<double>(0, 3), T_BS.at<double>(1, 3), T_BS.at<
			double>(2, 3);
	...
	initNoiseParam.var_gc = gyroscope_noise_density * gyroscope_noise_density
			* I;
	...
	initNoiseParam.var_ac = accelerometer_noise_density
			* accelerometer_noise_density * I;

	double accelerometer_random_walk;

	fs1["accelerometer_random_walk"] >> accelerometer_random_walk;

	initNoiseParam.var_wac = accelerometer_random_walk
			* accelerometer_random_walk * I;
...
std::vector<ImageNameStruct> getImageList(std::ifstream& imageListFile) {
	...
	while (std::getline(imageListFile, line) && !line.empty()) {
		if (line[0] == '#') // do not read comment lines.
				{
			continue;
		}
...

{
...

	{
		...
		std::sscanf(line.c_str(), "%lf,%lf,%lf,%lf,%lf,%lf,%lf", &timeStamp,
				&data.g.x(), &data.g.y(), &data.g.z(), &data.a.x(), &data.a.y(),
				&data.a.z());
...
std::vector<IMUGroundTruth> getGroundTruthEuroc(
...

	{
		...
		std::sscanf(line.c_str(),
				...
				&ts, &imuGroudTruth.p_b_G.x(), &imuGroudTruth.p_b_G.y(),
				&imuGroudTruth.p_b_G.z(),

				&imuGroudTruth.q_B_G(3), &imuGroudTruth.q_B_G(0),
				&imuGroudTruth.q_B_G(1), &imuGroudTruth.q_B_G(2),

				&imuGroudTruth.v_b_G.x(), &imuGroudTruth.v_b_G.y(),
				&imuGroudTruth.v_b_G.z(),

				&imuGroudTruth.bias_w(0), &imuGroudTruth.bias_w(1),
				&imuGroudTruth.bias_w(2),

				&imuGroudTruth.bias_a(0), &imuGroudTruth.bias_a(1),
				&imuGroudTruth.bias_a(2));
